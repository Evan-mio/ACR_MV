...
function validateTableNakeConsistency() {
    ...
    if (typeof BB_PRODUCKT_DATABASE === 'undefined') return;
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (grdInput && grdInput.value.trim() !== "") {
            ...
            const foundProd = BB_PRODUCKT_DATABASE.find(item => item.GRD === grdValue);
    ...
    rows.forEach((currentRow) => {
        ...
        if (isNaN(grdValue) || grdInput.value.trim() === "") {
            ...
            const currentProd = BB_PRODUCKT_DATABASE.find(item => item.GRD === grdValue);
...
function initDatalists() {
    ...
    fillList('linesList', BB_LINE_DATABASE, 'PacLine');
    fillList('grdList', BB_PRODUCKT_DATABASE, 'GRD', 'GRDName');
    fillList('carsList', BB_PAC_CAR_DATABASE, 'PacCar');
}
...
// ====================================================
// 6. ДОБАВЛЕНИЕ СТРОК В ТАБЛИЦУ ИНТЕРФЕЙСА (ИСПРАВЛЕННЫЙ)
// ====================================================
function addTableRows(count) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        const tr = document.createElement('tr');
        //  Убран атрибут readonly у поля no_car_, чтобы разрешить ручной ввод на всех строках
        tr.innerHTML = `
...
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        ...
        setTimeout(() => handleLiveChanges(event), 50); 
            ...
            if (typeof BB_PRODUCKT_DATABASE !== 'undefined') {
                const foundProd = BB_PRODUCKT_DATABASE.find(item => item.GRD === grdValue);
                if (foundProd) {
...
// =========================================================================
// 21. МНОГОШАГОВЫЙ СМАРТ-МАСТЕР ДЛЯ АКТА ССЫПКИ В БИГ-БЭГИ (ВЕРСИЯ 4.0)
// =========================================================================
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        // 1. Внедрение кнопки СТАРТ в контейнер вкладок бланка
        ...
        if (tabsContainer && !document.getElementById('btnFixEquipmentProcess')) {
            ...
            startBtn.innerText = '⚡ СТАРТ (Акт ссыпки)';
            startBtn.style.cssText = 'background-color:#f59e0b; color:white; padding:8px 16px; border-radius:8px; cursor:pointer; font-weight:600; font-size:13px; border:1px solid transparent; margin-left:10px; transition: all 0.2s;';
            
            // Плавный hover-эффект для кнопки
            startBtn.onmouseover = () => startBtn.style.backgroundColor = '#d97706';
            startBtn.onmouseout = () => startBtn.style.backgroundColor = '#f59e0b';
        ...
        // 2. Создание структуры многошагового модального окна мастера
        let modal = document.getElementById('process-wizard-modal');
        if (!modal) {
            ...
            modal.innerHTML = `
                <div style="background:#ffffff; padding:24px; border-radius:12px; width:95%; max-width:440px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1); max-height:90vh; display:flex; flex-direction:column;">
                    <h3 style="margin-bottom:16px; color:#0f172a; font-size:1.2rem; font-weight:700; border-bottom:2px solid #f3f4f6; padding-bottom:10px;">📦 Мастер ссыпки в Биг-Бэги</h3>
                    
                    <div style="flex:1; overflow-y:auto; margin-bottom:20px; padding-right:4px;">
                        
                        <!-- ШАГ 1: ВЫБОР ЛИНИИ -->
                        <div id="wiz-step-1">
                            <label style="display:block; margin-bottom:8px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase; letter-spacing:0.05em;">Шаг 1: Выберите линию ссыпки</label>
                            <select id="wiz-select-line" style="width:100%; padding:10px; border:1px solid #cbd5e1; border-radius:6px; font-size:14px; background:#fff; color:#000; outline:none; box-shadow: inset 0 1px 2px rgba(0,0,0,0.05);"></select>
                        </div>

                        <!-- ШАГ 2: ПАРАМЕТРЫ ПАРТИИ -->
                        <div id="wiz-step-2" style="display:none;">
                            <label style="display:block; margin-bottom:12px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase; letter-spacing:0.05em;">Шаг 2: Параметры партии ссыпки</label>
                            
                            <div style="display:flex; flex-direction:column; gap:14px;">
                                <!-- Выбор номера машины -->
                                <div>
                                    <label style="display:block; margin-bottom:4px; font-size:12px; color:#475569; font-weight:600;">Номер упаковочной машины:</label>
                                    <select id="wiz-select-car" style="width:100%; padding:8px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; background:#fff; color:#000; box-sizing:border-box; outline:none;"></select>
                                </div>

                                <!-- Выбор кода продукта (GRD) -->
                                <div>
                                    <label style="display:block; margin-bottom:4px; font-size:12px; color:#475569; font-weight:600;">Код продукта (GRD):</label>
                                    <select id="wiz-select-grd" style="width:100%; padding:8px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; background:#fff; color:#000; box-sizing:border-box; outline:none;"></select>
                                </div>

                                <!-- Количество добавляемых мешков -->
                                <div>
                                    <label style="display:block; margin-bottom:4px; font-size:12px; color:#475569; font-weight:600;">Сколько мешков (строк) внести:</label>
                                    <input type="number" id="wiz-input-rows-count" min="1" max="15" value="1" style="width:100%; padding:8px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; background:#fff; color:#000; box-sizing:border-box; outline:none;">
                                    <small style="color:#94a3b8; font-size:11px; display:block; margin-top:4px;">Максимальная емкость одного листа ограничена до 15 строк.</small>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="display:flex; justify-content:flex-end; gap:10px; border-top:1px solid #e2e8f0; padding-top:15px; flex-shrink:0;">
                        ...
                        <button type="button" id="wiz-btn-close" style="display:none; padding:8px 16px; background:#64748b; color:#fff; border:none; border-radius:6px; font-weight:600; cursor:pointer; font-size:13px;">Закрыть</button>
        ...
        if (startBtnReal) {
            startBtnReal.addEventListener('click', (e) => {
                ...
                document.getElementById('wiz-input-rows-count').value = '1';
                switchStep(1);
                modal.style.display = 'flex';
            });
        }

        // Подгрузка баз данных в выпадающие списки (select)
        function buildWizardDropdowns() {
            const lineSelect = document.getElementById('wiz-select-line');
            if (lineSelect && lineSelect.children.length === 0 && typeof BB_LINE_DATABASE !== 'undefined') {
                BB_LINE_DATABASE.forEach(item => {
                    if (item.PacLine) lineSelect.add(new Option(item.PacLine, item.PacLine));
                });
            }

            const carSelect = document.getElementById('wiz-select-car');
            if (carSelect && carSelect.children.length === 0 && typeof BB_PAC_CAR_DATABASE !== 'undefined') {
                BB_PAC_CAR_DATABASE.forEach(item => {
                    if (item.PacCar) carSelect.add(new Option(item.PacCar, item.PacCar));
                });
            }

            const grdSelect = document.getElementById('wiz-select-grd');
            if (grdSelect && grdSelect.children.length === 0 && typeof BB_PRODUCKT_DATABASE !== 'undefined') {
                BB_PRODUCKT_DATABASE.forEach(item => {
                    if (item.GRD) {
        ...
        // Переключение экранов (Шаг 1 и Шаг 2)
        function switchStep(stepNum) {
            ...
            // Видимость кнопок на Шаге 2
            document.getElementById('wiz-btn-back').style.display = stepNum === 2 ? 'block' : 'none';
            document.getElementById('wiz-btn-close').style.display = stepNum === 2 ? 'block' : 'none';
            document.getElementById('wiz-btn-save').style.display = stepNum === 2 ? 'block' : 'none';
        }

        // Инициализация событий внутренних кнопок мастера
        function initWizardEvents() {
            ...
            document.getElementById('wiz-btn-close').addEventListener('click', () => modal.style.display = 'none');
            
            // Главное действие: распределение данных по ячейкам таблицы
            document.getElementById('wiz-btn-save').addEventListener('click', () => {
                const lineVal = document.getElementById('wiz-select-line').value;
                const carVal = document.getElementById('wiz-select-car').value;
                const grdVal = document.getElementById('wiz-select-grd').value;
                let rowsToAllocate = parseInt(document.getElementById('wiz-input-rows-count').value, 10);
                
                if (!tableBody) return;
                if (isNaN(rowsToAllocate) || rowsToAllocate < 1 || rowsToAllocate > 15) {
                    alert("Укажите корректное количество строк (от 1 до 15)!");return;
                }
                
                // Шаг А: Сначала ищем пустые строки, которые уже отрендерены на листе
                let existingRows = tableBody.querySelectorAll('tr');
                for (let i = 0; i < existingRows.length; i++) {
                    if (rowsToAllocate <= 0) break;
                    
                    const lineInp = existingRows[i].querySelector('input[name^="no_line_"]');
                    const grdInp = existingRows[i].querySelector('input[name^="grd_"]');
                    
                    // Если строка чистая по ключевым параметрам — занимаем её под ссыпку
                    if (lineInp && !lineInp.value.trim() && grdInp && !grdInp.value.trim()) {
                        fillTargetRowValues(existingRows[i], lineVal, carVal, grdVal);
                        rowsToAllocate--;
                    }
                }
                
                // Шаг Б: Если пустых ячеек не хватило, автоматически генерируем новые строки до лимита в 15 шт.
                if (rowsToAllocate > 0) {
                    const currentTotal = tableBody.querySelectorAll('tr').length;
                    if (currentTotal + rowsToAllocate > 15) {
                        alert(`Достигнут жесткий лимит бланка! Будет добавлено только ${15 - currentTotal} строк(и).`);
                        rowsToAllocate = 15 - currentTotal;
                    }
                    
                    for (let r = 0; r < rowsToAllocate; r++) {
                        if (typeof addTableRows === 'function') {
                            addTableRows(1); // Используем стандартный генератор строк из Блока 6
                        }
                        
                        const currentRows = tableBody.querySelectorAll('tr');
                        const newRow = currentRows[currentRows.length - 1];
                        if (newRow) {fillTargetRowValues(newRow, lineVal, carVal, grdVal);
                        }
                    }
                }
                
                // Принудительно обновляем пробы и крестики удаления для свежих данных
                if (typeof validateTableNakeConsistency === 'function') validateTableNakeConsistency();
                attachDeleteButtonsToTable();
                modal.style.display = 'none';
            });
        }
        
        // Вспомогательная функция записи в целевые ячейки (Линия, Машина, GRD)
        function fillTargetRowValues(row, line, car, grd) {
            const lineInput = row.querySelector('input[name^="no_line_"]');
            const grdInput = row.querySelector('input[name^="grd_"]');
            const carInput = row.querySelector('input[name^="no_car_"]');
            
            if (lineInput) lineInput.value = line;
            if (grdInput) grdInput.value = grd;
            if (carInput) {
                carInput.value = car;
                if (typeof updateRowLot === 'function') updateRowLot(row); // Рассчитываем готовый лот продукции
            }
            
            // Вызываем цепочку нативных событий 'input'/'change' для срабатывания остальных блоков
            ...
            // Зеленая анимация-подсветка успешного добавления
    ...
    // Автоматический контроль кнопок удаления при инициализации страницы и вкладок
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(attachDeleteButtonsToTable, 500);
        
        const originalLoadSheetData = window.loadSheetData;
        window.loadSheetData = function(sheetId) {
            if (typeof originalLoadSheetData === 'function') originalLoadSheetData(sheetId);
            attachDeleteButtonsToTable();// Перепривязка крестиков при смене табов
            };
        });
        
        // ГЛАВНАЯ ФУНКЦИЯ ИНТЕГРАЦИИ КНОПОК УДАЛЕНИЯ СТРОК
        window.attachDeleteButtonsToTable = function() {
            const tBody = document.getElementById('table-body');
            if (!tBody) return;
            
            const rows = tBody.querySelectorAll('tr');
            rows.forEach(row => {
                // Защита от дублирования кнопок удаления в одной ячейке
                if (row.querySelector('.btn-delete-row-action')) return;
                const lastTd = row.lastElementChild; 
                
                // Колонка Проба ("proba_")
                if (lastTd) {
                    ...
                    delBtn.type = 'button';
                    delBtn.className = 'btn-delete-row-action';
                    delBtn.innerText = '❌';
                    delBtn.title = 'Удалить этот мешок со сдвигом данных вверх';
                    delBtn.style.cssText = 'padding: 2px 6px; background: #ef4444; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: 11px; font-weight: bold; line-height: 1; height: 24px; transition: background 0.2s; flex-shrink: 0;';
                    
                    delBtn.onmouseover = () => delBtn.style.background = '#dc2626';
                    delBtn.onmouseout = () => delBtn.style.background = '#ef4444';
                    
                    delBtn.addEventListener('click', (e) => {
                        e.preventDefault();
                        if (confirm('Вы уверены, что хотите безвозвратно удалить эту строку? Все данные ниже сдвинутся вверх.')) {
                            if (typeof deleteTableRowWithShift === 'function') {
                                deleteTableRowWithShift(row);
                                
                                // Каскадное сквозное обновление номеров проб и оставшихся кнопок
                                if (typeof validateTableNakeConsistency === 'function') validateTableNakeConsistency();
                                if (typeof updateAllProbaValues === 'function') updateAllProbaValues();
                                attachDeleteButtonsToTable();
                            }
                        }
                    });
