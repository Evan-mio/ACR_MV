...
// =========================================================================
// 21. МНОГОШАГОВЫЙ СМАРТ-МАСТЕР ДЛЯ АКТА ССЫПКИ В БИГ-БЭГИ (ВЕРСИЯ 3.0)
// =========================================================================
(function() {
    document.addEventListener('DOMContentLoaded', () => {
        // 1. Внедрение кнопки СТАРТ в контейнер вкладок
        ...
        // 2. Создание двухшагового модального окна мастера
        ...
        modal.innerHTML = `
            <div style="background:#ffffff; padding:24px; border-radius:12px; width:95%; max-width:420px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1); max-height:90vh; display:flex; flex-direction:column;">
                <h3 style="margin-bottom:16px; color:#0f172a; font-size:1.15rem; font-weight:700;">📦 Мастер ссыпки в биг-бэги</h3>
                
                <div style="flex:1; overflow-y:auto; margin-bottom:20px; padding-right:4px;">
                    
                    <!-- ОКНО 1: ВЫБОР ЛИНИИ -->
                    <div id="wiz-step-1">
                        <label style="display:block; margin-bottom:6px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">Окно 1: Выберите Линию Ссыпки</label>
                        <select id="wiz-select-line" style="width:100%; padding:10px; border:1px solid #cbd5e1; border-radius:6px; font-size:14px; background:#fff; color:#000;"></select>
                    </div>

                    <!-- ОКНО 2: МАШИНА, ПРОДУКТ И СТРОКИ -->
                    <div id="wiz-step-2" style="display:none;">
                        <label style="display:block; margin-bottom:12px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">Окно 2: Параметры партии ссыпки</label>
                        
                        <div style="display:flex; flex-direction:column; gap:14px;">
                            <!-- Поле Номер машины -->
                            <div>
                                <label style="display:block; margin-bottom:4px; font-size:12px; color:#475569; font-weight:600;">Номер машины:</label>
                                <input type="text" id="wiz-input-car" list="carsList" placeholder="Введите или выберите номер машины" style="width:100%; padding:8px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; background:#fff; color:#000; box-sizing:border-box;">
                            </div>

                            <!-- Поле Код продукта -->
                            <div>
                                <label style="display:block; margin-bottom:4px; font-size:12px; color:#475569; font-weight:600;">Код продукта (GRD):</label>
                                <select id="wiz-select-grd" style="width:100%; padding:8px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; background:#fff; color:#000; box-sizing:border-box;"></select>
                            </div>

                            <!-- Количество строк -->
                            <div>
                                <label style="display:block; margin-bottom:4px; font-size:12px; color:#475569; font-weight:600;">Сколько мешков (строк) добавить:</label>
                                <input type="number" id="wiz-input-rows-count" min="1" max="15" value="1" style="width:100%; padding:8px; border:1px solid #cbd5e1; border-radius:6px; font-size:13px; background:#fff; color:#000; box-sizing:border-box;">
                                <small style="color:#94a3b8; font-size:11px; display:block; margin-top:4px;">Максимальная емкость листа — 15 строк.</small>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        if (startBtnReal) {
            startBtnReal.addEventListener('click', (e) => {
                ...
                document.getElementById('wiz-input-car').value = '';
        ...
        // Подгрузка баз данных при инициализации
        function buildWizardDropdowns() {
            const lineSelect = document.getElementById('wiz-select-line');
            if (lineSelect && lineSelect.children.length === 0 && typeof PAC_LINES_DATABASE !== 'undefined') {
                PAC_LINES_DATABASE.forEach(item => {
            ...
            if (grdSelect && grdSelect.children.length === 0 && typeof NAKE_PRODUCKT_DATABASE !== 'undefined') {
                NAKE_PRODUCKT_DATABASE.forEach(item => {
                    if (item.GRD) {
        ...
        document.getElementById('wiz-btn-save').addEventListener('click', () => {
            ...
            if (!tableBody) return;
            if (isNaN(rowsToAllocate) || rowsToAllocate < 1) {
                alert("Укажите корректное количество строк!");
                return;
            }

            // ШАГ А: Сначала ищем пустые строки, которые УЖЕ есть на листе (включая самую первую)
            ...
            for (let i = 0; i < existingRows.length; i++) {
                ...
                // Если строка полностью чистая по ключевым параметрам — занимаем её
                if (lineInp && !lineInp.value.trim() && grdInp && !grdInp.value.trim()) {
                    fillTargetRowValues(existingRows[i], lineVal, carVal, grdVal);
                    rowsToAllocate--; // Уменьшаем остаток мешков для распределения
                }
            }
            
            // ШАГ Б: Если пустых строк не хватило, генерируем новые физические строки до лимита в 15 шт.
            if (rowsToAllocate > 0) {
                const currentTotal = tableBody.querySelectorAll('tr').length;
                if (currentTotal + rowsToAllocate > 15) {
                    alert(`Внимание! Будет добавлено только часть строк. 
                        Общий лимит листа жестко ограничен до 15 строк!`);
                    rowsToAllocate = 15 - currentTotal; // Корректируем до максимума
                }
                
                for (let r = 0; r < rowsToAllocate; r++) {
                    if (typeof addTableRows === 'function') {
                        addTableRows(1); // Вызываем вашу стандартную функцию добавления строки
                    }
            ...
            // Навешиваем кнопки удаления на всю таблицу, чтобы новые строки тоже их получили
            attachDeleteButtonsToTable();
            modal.style.display = 'none';
        });
        
        // Вспомогательная функция изолированной записи в 3 колонки
        function fillTargetRowValues(row, line, car, grd) {
            ...
            // Записываем данные только в 3 требуемые колонки. Колонки времени ("cas_", "min_") не трогаем!
            if (lineInput) lineInput.value = line;
            if (grdInput) grdInput.value = grd;
            if (carInput) {
                carInput.value = car;
                if (typeof updateRowLot === 'function') updateRowLot(row); // Сразу рассчитываем БАТЧ-ЛОТ готовой продукции
            }
            
            // Триггерим события для автоматических расчетов Найка по цепочке
            ...
            // Световой индикатор успешной обработки строки мастера
            row.style.transition = "background-color 0.4s";
            row.style.backgroundColor = "#dcfce7"; // Мягкий зеленый
            setTimeout(() => row.style.backgroundColor = "", 1000);
        }
    });
    
    // Автоматический запуск навешивания кнопок удаления при старте бланка
    document.addEventListener('DOMContentLoaded', () => {
        setTimeout(attachDeleteButtonsToTable, 500);
        
        // Интеграция с переключением вкладок: добавляем кнопки при загрузке данных листа из памяти
        const originalLoadSheetData = window.loadSheetData;
        window.loadSheetData = function(sheetId) {
            if (typeof originalLoadSheetData === 'function') originalLoadSheetData(sheetId);
            attachDeleteButtonsToTable();
    ...
    window.attachDeleteButtonsToTable = function() {
        ...
        const rows = tBody.querySelectorAll('tr');
        rows.forEach(row => {
            // Проверяем, есть ли уже кнопка удаления в этой строке, чтобы не дублировать её
            if (row.querySelector('.btn-delete-row-action')) return;
            
            // Ищем последнюю ячейку (колонка Проба "proba_")
            const lastTd = row.lastElementChild;
            if (lastTd) {
                // Превращаем ячейку в flex-контейнер для аккуратного размещения кнопки рядом с инпутом
                lastTd.style.display = 'flex';
                lastTd.style.alignItems = 'center';
                lastTd.style.gap = '6px';
                
                // Создаем компактную кнопку удаления мешка
                const delBtn = document.createElement('button');
                delBtn.type = 'button';
                delBtn.className = 'btn-delete-row-action';
                delBtn.innerText = '❌';
                delBtn.title = 'Удалить эту строку со сдвигом данных наверх';
                delBtn.style.cssText = 'padding: 2px 6px; background: #ef4444; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-size: 11px; font-weight: bold; line-height: 1; height: 24px; transition: background 0.2s;';
                
                delBtn.onmouseover = () => delBtn.style.background = '#dc2626';
                delBtn.onmouseout = () => delBtn.style.background = '#ef4444';
                
                // Привязываем клик к вашей заводской системе безопасного сдвига ячеек
                delBtn.addEventListener('click', (e) => {
                    e.preventDefault();
                    if (confirm('Вы уверены, что хотите безвозвратно удалить эту строку? Данные ниже будут сдвинуты вверх.')) {
                        if (typeof deleteTableRowWithShift === 'function') {
                            deleteTableRowWithShift(row);
                            // После удаления пересобираем кнопки для надежности
                            attachDeleteButtonsToTable();
                        }
                    }
                });
