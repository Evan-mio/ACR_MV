    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const handleLiveChangesLocal = (event) => {
            ...
            if (target.name.startsWith('grd_')) {
                ...
                if (!isNaN(grdValue) && typeof NAKE_PRODUCKT_DATABASE !== 'undefined') {
                    const foundProd = NAKE_PRODUCKT_DATABASE.find(item => item.GRD === grdValue);
                    if (foundProd) {
...
// =========================================================================
// 21. ТРЕХШАГОВЫЙ СМАРТ-МАСТЕР (ШАГ 1: ВРЕМЯ -> ШАГ 2: ЛИНИЯ -> ШАГ 3: МАШИНЫ)
// =========================================================================
(function() {
    // Встроенный тайм-менеджер автоматического определения интервала, если время не выбрано вручную
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        // Динамическое создание и вывод кнопки запуска мастера внутри вкладок
const tabsContainer = document.querySelector('.tabs-container');
if (tabsContainer && !document.getElementById('btnFixEquipmentProcess')) {
    const wizardBtn = document.createElement('button');
    wizardBtn.type = 'button';
    wizardBtn.id = 'btnFixEquipmentProcess';
    wizardBtn.className = 'btn btn-warning'; // Оранжевый цвет по вашей CSS-базе
    wizardBtn.innerText = '⚡ Фиксация процесса';
    
    // Вставляем кнопку в блок вкладок вторым элементом (сразу после списка листов #tab-list)
    const tabListEl = document.getElementById('tab-list');
    if (tabListEl && tabListEl.nextSibling) {
        tabsContainer.insertBefore(wizardBtn, tabListEl.nextSibling);
    } else {
        tabsContainer.appendChild(wizardBtn);
    }
}

        // Создание трехшаговой модалки в DOM
        ...
        modal.innerHTML = `
            <div style="background:#ffffff; padding:24px; border-radius:12px; width:95%; max-width:540px; box-shadow:0 20px 25px -5px rgba(0,0,0,0.1); max-height:90vh; display:flex; flex-direction:column;">
                <h3 style="margin-bottom:16px; color:#0f172a; font-size:1.2rem; font-weight:700;">⚡ Фиксация упаковочного процесса</h3>
                
                <div style="flex:1; overflow-y:auto; margin-bottom:20px; padding-right:4px;">
                    
                    <!-- ШАГ 1: ВЫБОР ВРЕМЕНИ ОТБОРА -->
                    <div id="wiz-step-1">
                        <label style="display:block; margin-bottom:6px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">Шаг 1: Выберите время отбора фактическое</label>
                        <div style="display:grid; grid-template-columns: 1fr 1fr; gap:15px; margin-bottom:15px;">
                            <div>
                                <label style="font-size:10px; color:#475569;">Час (00-23):</label>
                                <select id="wiz-select-hour" style="width:100%; padding:8px; border:1px solid #7b7d80; border-radius:6px; background:#fff; color:#000;"></select>
                            </div>
                            <div>
                                <label style="font-size:10px; color:#475569;">Минуты (00-59):</label>
                                <select id="wiz-select-minute" style="width:100%; padding:8px; border:1px solid #7b7d80; border-radius:6px; background:#fff; color:#000;"></select>
                            </div>
                        </div>
                        <p style="font-size:0.8rem; color:#64748b; background:#f1f5f9; padding:8px; border-radius:6px;">💡 Примечание: Вы можете выбрать точное время. Если оставить поля пустыми, система подставит текущее округленное время по графику технологических смен.</p>
                    </div>

                    <!-- ШАГ 2: ВЫБОР ЛИНИИ -->
                    <div id="wiz-step-2" style="display:none;">
                        <label style="display:block; margin-bottom:6px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">Шаг 2: Выберите Упаковочную Линию</label>
                    ...
                    <!-- ШАГ 3: ВЫБОР СВЯЗОК МАШИН И КОДОВ (С КНОПКОЙ ПЛЮС) -->
                    <div id="wiz-step-3" style="display:none;">
                        <label style="display:block; margin-bottom:6px; font-size:11px; font-weight:700; color:#64748b; text-transform:uppercase;">Шаг 3: Упаковочные машины и коды продуктов</label>
                        <p style="font-size:0.85rem; color:#64748b; margin-bottom:12px;">Сформируйте список работающего оборудования для выбранной линии:</p>
                        ...
                        <button type="button" id="wiz-btn-add-machine" class="btn" style="background-color:#f1f5f9; color:#1e293b; border:1px dashed #cbd5e1; width:100%; padding:8px; font-size:12px; border-radius:6px; cursor:pointer; font-weight:600;">➕ Добавить еще одну машину к этой линии</button>
                    </div>
                </div>
        ...
        // Отслеживание клика по кнопке СТАРТ
        document.addEventListener('click', (e) => {
            if (e.target && e.target.id === 'btnFixEquipmentProcess') {
                e.preventDefault();
        ...
        // Сборка списков времени и линий при запуске
        function buildWizardDropdowns() {
            // Наполнение Часов (Шаг 1)
            const hourSelect = document.getElementById('wiz-select-hour');
            if (hourSelect && hourSelect.children.length === 0) {
                hourSelect.add(new Option("--Автоматически--", ""));
                for (let h = 0; h < 24; h++) {
                    const hStr = String(h).padStart(2, '0');
                    hourSelect.add(new Option(hStr, hStr));
                }
            }

            // Наполнение Минут (Шаг 1)
            const minSelect = document.getElementById('wiz-select-minute');
            if (minSelect && minSelect.children.length === 0) {
                minSelect.add(new Option("--Автоматически--", ""));
                for (let m = 0; m < 60; m++) {
                    const mStr = String(m).padStart(2, '0');
                    minSelect.add(new Option(mStr, mStr));
                }
            }

            // Наполнение Линий (Шаг 2)
...
if (lineSelect && lineSelect.children.length === 0 && typeof PAC_LINES_DATABASE !== 'undefined') {
    PAC_LINES_DATABASE.forEach(item => {
        if (item.PacLine) {
            // Создаем опцию стандартным способом и безопасно добавляем её в дерево
            const option = new Option(item.PacLine, item.PacLine);
            ...
            // Инициализация первой связки оборудования (Шаг 3)
        ...
        // Динамическое добавление машины и кодов (Логика кнопки ПЛЮС)
        function addEquipmentRow() {
            const container = document.getElementById('wiz-equipment-rows-container');
            if (!container) return;
            
            if (container.children.length >= 12) {
                alert("Ограничение: одна упаковочная линия поддерживает до 12 одновременно работающих машин.");
                return;
            }
            ...
            // Селект упаковочной машины
            const carBlock = document.createElement('div');
            carBlock.innerHTML = 
            `<label style="display:block; margin-bottom:4px; font-size:10px; color:#475569; font-weight:700;">Машина:</label>`;
            ...
            if (typeof PAC_CAR_DATABASE !== 'undefined') {
                PAC_CAR_DATABASE.forEach(item => {
            ...
            grdBlock.innerHTML = 
            `<label style="display: block; margin-bottom: 4px; font-size: 10px; color:#475569; font-weight:700;">Код продукта (GRD):</label>`;
            ...
            // Кнопка удаления связки внутри окна
            ...
            delBtn.onclick = () => {
                ...
                else alert("Требуется зафиксировать хотя бы одно работающее устройство.");
            };
        ...
        // Переключатель шагов (Вперед / Назад)
        function switchStep(stepNum) {
            ...
            document.getElementById('wiz-step-3').style.display = stepNum === 3 ? 'block' : 'none';
            
            // Видимость кнопок навигации
            document.getElementById('wiz-btn-cancel').style.display = stepNum === 1 ? 'block' : 'none';
            document.getElementById('wiz-btn-next').style.display = (stepNum === 1 || stepNum === 2) ? 'block' : 'none';
            document.getElementById('wiz-btn-back').style.display = (stepNum === 2 || stepNum === 3) ? 'block' : 'none';
            document.getElementById('wiz-btn-save').style.display = stepNum === 3 ? 'block' : 'none';
        }
        
        // Обработчики кнопок шагов
        document.getElementById('wiz-btn-add-machine').addEventListener('click', (e) => { 
            e.preventDefault(); addEquipmentRow(); 
        });
        document.getElementById('wiz-btn-next').addEventListener('click', () => {
            if (activeStep === 1) switchStep(2);
            else if (activeStep === 2) switchStep(3);
        });
        document.getElementById('wiz-btn-back').addEventListener('click', () => {
            if (activeStep === 3) switchStep(2);
            else if (activeStep === 2) switchStep(1);
        });
        document.getElementById('wiz-btn-cancel').addEventListener('click', () => 
            modal.style.display = 'none');
        
        // КНОПКА: Внести данные (Итоговая выгрузка по всем трем параметрам)
        document.getElementById('wiz-btn-save').addEventListener('click', () => {
            // Читаем время (Шаг 1)
            const chosenHour = document.getElementById('wiz-select-hour').value;
            const chosenMinute = document.getElementById('wiz-select-minute').value;
            
            let finalHour, finalMinute;
            if (chosenHour !== "" && chosenMinute !== "") {
                finalHour = chosenHour;
                finalMinute = chosenMinute;
            } else {
                // Если не выбрано — берем квантизированное по сменам
                const [qHour, qMinute] = getQuantizedTime();
                finalHour = qHour;finalMinute = qMinute;
            }
            
            // Читаем линию (Шаг 2)
            const lineVal = document.getElementById('wiz-select-line').value;
            // Читаем матрицу оборудования (Шаг 3)
            ...
            eqRows.forEach(rowBlock => {
                ...
                // Сканируем таблицу бланка на наличие пустых ячеек
                for (let i = 0; i < rows.length; i++) {
                ...
                // Автогенерация строки, если свободное место на листе кончилось
                if (!targetRow) {
                ...
                // Запись накопленной информации в ячейки бланка
                if (targetRow) {
                    ...
                    // Запись фактического времени из Шага 1
                    if (casInput) casInput.value = finalHour;
                    if (minInput) minInput.value = finalMinute;
                    ...
                    // Передача событий для запуска пересчета лотов, Найков и проб
